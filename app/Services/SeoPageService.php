...
class SeoPageService
{
    ...
    public function replace(string $subject, $subSearch = [], $subReplace = []): string
    {
        $search = [
            '{domen}',
            '{url}',
        ...
        $sessionGeo = session('geo');

        $replace = [
            config('app.domain'),
            config('app.url'),
            $sessionGeo['country'] ?? '',
            $sessionGeo['city_name'] ?? '',
            $sessionGeo['region_name'] ?? '',
            config('app.name'),
            $sessionGeo['country_declination'] ?? '',
            ...$subReplace,
            ' ',
        ];
    ...
    public function home(): array
    {
        ...
        if ($page) {
            foreach ($value as $key => $val) {
                $value[$key] = $this->replace((string)$page->$key);
            }
        }
    ...
    public function catalog(object $data = null): array
    {
        // {board_main_categories} {board_category_name} {board_category_title} {board_category_h1} {board_category_meta_desc} {board_category_text}
        $search = [
        ...
        $replace = [
            $this->categoryRepository->mainCategoriesText(),
            $data->category_board_name,
            $data->category_board_title,
            $data->category_board_h1,
            $data->category_board_description,
            $data->category_board_text
        ];

        return [
            'meta_title' => $this->replace($data->category_board_title, $search, $replace),
            'meta_desc' => $this->replace($data->category_board_description, $search, $replace),
            'text' => $this->replace($data->category_board_text, $search, $replace),
            'h1' => $this->replace($data->category_board_h1, $search, $replace),
        ];
    }

    public function out($param = [], $data = [])
    {
        $lang_iso = app()->getLocale();
        ...
        if (isset($param['page'])) {
            ...
            if (count($get)) {
                if (isset($get[$param['field']])) {
                    if (!isset($param['text'])) {
                        return str_replace($search, $values, $get[$param['field']]);
                    } else {
                        $replace = str_replace($search, $values, $get[$param['field']]);
                        return str_replace($search, $values, $replace);
                    }
                }

            }
        } else {
            if (isset($param['text'])) return str_replace($search, $values, $param['text']);
        }
    }
}
